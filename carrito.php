...
function obtener_descuento_aplicable($producto_id, $cantidad, $descuentos)
{
    foreach ($descuentos as $descuento) {
        // Si el descuento es específico para un producto, se verifica que el producto coincida
        if (($descuento['producto_id'] == NULL || $descuento['producto_id'] == $producto_id) && $cantidad >= $descuento['cantidad_minima']) {
            return $descuento['porcentaje_descuento'];  // Retorna el porcentaje de descuento si es válido
        }
    }
...
foreach ($carrito as $key => $item) {
    ...
    // Actualizar el precio en el carrito con descuento
    $carrito[$key]['precio_con_descuento'] = $precio_con_descuento;
}
            ...
            <?php if (!empty($carrito)): ?>
                <?php foreach ($carrito as $item): ?>
                    <tr data-id="<?= $item['carrito_id'] ?>">
                        ...
                        <td>
                            <?php if ($item['precio_con_descuento'] < $item['precio']): ?>
                                Descuento: $<?= number_format($item['precio'] - $item['precio_con_descuento'], 2); ?>
                            <?php else: ?>
                                No aplica descuento
                            <?php endif; ?>
                        </td>
                        <td class="subtotal">$<?= number_format($item['precio_con_descuento'] * $item['cantidad'], 2); ?></td>
                        <td>
                ...
                <?php endforeach; ?>
                <tr>
                    <td colspan="3" style="text-align: right;"><strong>Total con descuento:</strong></td>
